...
{
    ArenaAllocator_t *arena = (ArenaAllocator_t *)malloc(sizeof(ArenaAllocator_t));
    if (arena == NULL)
    {
        return NULL;
    }
...
{
    if (arena->memory == NULL || arena->next_offset + size > arena->capacity || arena->capacity == 0)
    {
        return NULL;
    }
...
void ArenaAllocator_Reset(ArenaAllocator_t *arena)
{
    if (arena == NULL || arena->memory == NULL)
    {
        return;

...
int main() {
  ...
  usize                file_size = yoru_get_file_size(filepath);
  printf("file is %zu bytes long\n", file_size);

  usize       offset                  = 10;
  usize       max_bytes               = 13;
  Yoru_String content                 = yoru_read_file(&allocator, filepath);
  Yoru_String content_skip_10_take_13 = yoru_read_file_exact(&allocator, filepath, offset, max_bytes);
  assert(content.data);
